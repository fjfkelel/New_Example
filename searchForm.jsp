...
$(document).ready(function(){
      ...
      $("[name=min_approval_year]").val("${param.min_approval_year}");
      $("[name=min_approval_month]").val("${param.min_approval_month}");
      $("[name=max_approval_year]").val("${param.max_approval_year}");
      $("[name=max_approval_month]").val("${param.max_approval_month}");
      $("[name=min_expense_year]").val("${param.min_expense_year}");
      $("[name=min_expense_month]").val("${param.min_expense_month}");
      $("[name=max_expense_year]").val("${param.max_expense_year}");
      $("[name=max_expense_month]").val("${param.max_expense_month}");
      $("[name=approval_rejection]").val("${param.approval_rejection}");
      $("[name=expense_means]").val("${param.expense_means}");
      $("[name=expense_area]").val("${param.expense_area}");
      $("[name=cost_min]").val("${param.cost_min}");
      $("[name=cost_max]").val("${param.cost_max}");
      $("[name=keyword1]").val("${param.keyword1}");
      $("[name=keyword2]").val("${param.keyword2}");
      $("[name=or_and]").val("${param.or_and}");
...
//----------------------------------------------------------------------------------------------------- 
      ...
      function goExpense_report_reg_form( ){
         //name=contentRegForm 을 가진 form 태그 안의 action에 설정된 url로 이동하기 
      ...
      function checkApprovalDate(){
       ...
       var expense_report_Obj = $("[name=expense_report]");
      ...
      function goExpenseContentForm( b_no ){
   ...
   <BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <input type="button" value="초기화 후 모두검색" class="searchAll">&nbsp;&nbsp;
       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
     ...
     <c:forEach var="expense" items="${expenseboardList}" varStatus="loopTagStatus"> 
         <tr align=center style="cursor:pointer" onClick="goExpenseContentForm(${expense.report_no});">
         <td>${beginRowNo_desc - loopTagStatus.index}
         <td align=left>
            ${expense.expense_name}
         <td>${expense.expense_field}
         <td>${expense.sign_code}
         <td>${expense.sign_date}
         <td>${expense.expense_year}${expense.expense_month}${expense.expense_date}
   ...
   <c:if test="${empty contactList}">
      ...
      <!-- 검색 결과물의 개수가 1개 이상이면 -->
         <c:if test="${expenseboardListAllCnt>0}">
            <!-- 만약 [시작페이지 번호]가 [화면당 보이는 페이지 번호 갯수]보다 크면 -->
            <!-- [처음] , [이전] 글씨 보이게 하고 , 클릭하면 원하는 페이지번호를 -->
            <!-- hidden 입력양식에 담고  현재 페이지를 다시 부르기  -->
            <c:if test="${beginPageNo>pageNoCntPerPage}">
               <a href="javascript:$('[name=selectPageNo]').val('1'goExpense_report()">[처음]</a>
               <a href="javascript:$('[name=selectPageNo]').val('${beginPageNo-1}');goExpense_report();">
                  [이전]&nbsp;&nbsp;
               </a>
            </c:if>
            <!-- [시작페이지 번호]부터 [마지막 페이지 번호]까지 페이지번호를 출력하기 -->
            <!-- 선택된 페이지 번호는 번호만 출력하고 클릭하지 못하게 한다. -->
            <!-- 선택 안된 페이지 번호는 [번호] 형태로 출력하고 클릭하면 원하는 페이지번호를 -->
            <!-- hidden 입력양식에 담고 현재 페이지를 다시 부르기 -->
            <c:forEach var="pageNo" begin="${beginPageNo}" end="${endPageNo}">
               <c:if test="${selectPageNo!=pageNo}">
                  <a href="javascript:$('[name=selectPageNo]').val('${pageNo}');goExpense_report();">
                     [${pageNo}]
                  </a>
               </c:if>
               <c:if test="${selectPageNo==pageNo}">
            ...
            <!-- 만약 [끝 페이지 번호]가 [맨 마지막 페이지 번호]보다 작으면 -->
            <!-- [다음] , [마지막] 글씨 보이게 하고 , 클릭하면 원하는 페이지번호를 -->
            <!-- hidden 입력양식에 담고  현재 페이지를 다시 부르기  -->
            <c:if test="${endPageNo<lastPageNo}">
               <a href="javascript:$('[name=selectPageNo]').val('${endPageNo+1}');goExpense_report();">
                  &nbsp;&nbsp;[다음]
               </a>
               <a href="javascript:$('[name=selectPageNo]').val('${lastPageNo}');goExpense_report();">
                  [마지막]
               </a>
            </c:if>
         </c:if>
      ...
      <form name="expenseContentForm" method="post" action ="/pro/expenseContentForm.do">
